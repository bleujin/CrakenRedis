...
final class ParserInternals {
	...
	static boolean many(final Parser<?> parser, final ParseContext ctxt) {
		for (int at = ctxt.at, step = ctxt.step;; step = ctxt.step) {
			if (!greedyRun(parser, ctxt))
				return stillThere(ctxt, at, step);
			int at2 = ctxt.at;
			if (at == at2)
				return true;
			at = at2;
	...
	static <T> boolean repeatAtMost(Parser<? extends T> parser, int max, Collection<T> collection, ParseContext ctxt) {
		for (int i = 0; i < max; i++) {
			int at = ctxt.at;
			int step = ctxt.step;
			if (!greedyRun(parser, ctxt))
	...
	static boolean stillThere(ParseContext ctxt, int wasAt, int originalStep) {
		if (ctxt.step == originalStep) {
			// logical step didn't change, so logically we are still there, undo
			// any physical offset
	...
	static boolean runNestedParser(ParseContext ctxt, ParseContext freshInitState, Parser<?> parser) {
		...
		// always copy error because there could be false alarms in the
		// character level.
		// For example, a "or" parser nested in a "many" failed in one of its
		// branches.
