...
/**
 * Curries the only public constructor defined in the {@code T} class and
 * invokes it with parameters returned by the sequentially executed
 * {@link Parser} objects. For example, to parse an expression with binary
 * operator and create an instance of the following object model:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The parser that parses this expression with binary operator can be written
 * as:
 * 
 * <pre>
 ...
 * {@link Unary} or {@link Binary} instance can be returned to cooperate with
 * {@link net.bleujin.rosetta.OperatorTable}, {@link Parser#prefix(Parser)},
...
final class Curry<T> extends Mapper<T> {
	...
	/**
	 * Creates a {@link Curry} object that curries the only public constructor
	 * of {@code clazz} with {@code curryArgs} by matching parameter types.
	 */
	...
	/**
	 * Two {@link Curry} objects are equal only if they curry the same class and
	 * have equal curry arguments.
	 */
	@Override
	public int hashCode() {
	...
	private static int findCurryIndex(Constructor<?> constructor, Class<?>[] parameterTypes, int index, Object object) {
		for (int i = 0; i < parameterTypes.length; i++) {
			if (Reflection.isInstance(parameterTypes[i], object))
				return i;
		}
	...
	Object invoke(Object[] args) throws Throwable {
		if (args.length != expectedParams()) {
			throw new IllegalArgumentException(expectedParams() + " parameters expected, " + args.length + " provided: " + invokable);
		}
