...
class DefaultTypeStrategy implements TypeStrategy {
	...
	private StringArrayValueObject stringArrayPropertyAdaptor = new StringArrayValueObject() ;
	private CollectionValueObject collectionPropertyAdaptor = new CollectionValueObject() ;


	public boolean supported(Field field, ReadNode node){
		return node.hasProperty(field.getName()) || node.hasChild(field.getName()) ;
	}
	...
	public Boolean resolveAdaptor(Object target, Field field, ReadNode node) throws IllegalArgumentException, IllegalAccessException {
		...
		Object read = adaptor.read(this, field, node);
		if (field.getType() == int.class && read.getClass() == Long.class) {
			field.set(target, ((Long)read).intValue()) ;
			return Boolean.TRUE;
		}
		field.set(target, read) ;
	...
	private TypeAdaptor findAdaptor(Field field, ReadNode node) {
		...
		if (node.hasProperty(field.getName())){
			if (PRIMITIVE_TYPE.containsKey(field.getType())) {
				...
				return stringArrayPropertyAdaptor ;
		...
		} else if (node.hasRef(field.getName())) {
...
class StringArrayValueObject extends TypeAdaptor<Object> {

	@Override
	public Object read(TypeStrategy ts, Field field, ReadNode node) {
		return node.property(field.getName()).asStrings();
	}
}
...
class ReferencesAdaptor extends TypeAdaptor<Object>{
	...
	@Override
	public Object read(TypeStrategy ts, Field field, ReadNode node) {
		Iterator<ReadNode> refs = node.refs(field.getName()).iterator();
